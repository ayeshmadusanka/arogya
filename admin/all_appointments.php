...
<head>
    ...
    <title>Arogya | Appointments</title>
    <!-- Bootstrap Core CSS -->
                ...
                <div class="row">
                    <div class="col-12">
                            ...
                            <h4 class="card-title">Appointments</h4>
                                <h6 class="card-subtitle">List of all Appointments</h6>
...
$sql = "SELECT a.*, p.p_name AS patient_name, d.d_name AS doctor_name 
FROM appointment a
INNER JOIN patient p ON a.p_id = p.p_id
INNER JOIN doctor d ON a.d_id = d.d_id";
$query = mysqli_query($db, $sql);
if (!mysqli_num_rows($query) > 0) {
    echo '<td colspan="9"><center>No Appointments Found</center></td>';
} else {
    while ($appointment = mysqli_fetch_array($query)) {
        ...
        echo '<tr>
                ...
                <td>' . $appointment['a_time_slot'] . '</td>
                <td>' . $appointment['a_placedt'] . '</td>
                <td>' . $appointment['a_user_status'] . '</td>
                <td>' . $appointment['a_doctor_status'] . '</td>
                <td>
                    <a href="delete_appointment.php?p_id=' . $appointment['p_id'] . '" class="btn btn-danger btn-sm">Delete</a>              
                </td>
            </tr>';
    }
}
