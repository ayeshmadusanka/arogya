...
<head>
    ...
    <title>Arogya | Patient Appointments</title>
    <!-- Bootstrap Core CSS -->
...
if (isset($_GET['p_id']) && !empty($_GET['p_id'])) {
    ...
    $sql = "SELECT a.*, p.p_name AS patient_name, d.d_name AS doctor_name 
            ...
            WHERE a.p_id = $patient_id";
    $query = mysqli_query($db, $sql);

    if (!mysqli_num_rows($query) > 0) {
        echo '<td colspan="9"><center>No Appointments Found for this Patient</center></td>';
    } else {
        while ($appointment = mysqli_fetch_array($query)) {
            ...
            echo '<tr>
                    ...
                    <td>
                        <a href="delete_patient_appointment.php?p_id=' . $rows['p_id'] . '" class="btn btn-danger btn-sm">Delete</a>              
                    </td>
                </tr>';
        }
    }
