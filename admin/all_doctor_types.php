...
<head>
    ...
    <title>Arogya | Doctor Types</title>
    <!-- Bootstrap Core CSS -->
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
						     ...
						     <div class="card">
                            <div class="card-body">
                            <h4 class="card-title">Doctor Types</h4>
                                <h6 class="card-subtitle">List of Doctor Types</h6>
                                <button class="btn btn-sm btn-success" data-toggle="modal" data-target="#addDoctorTypeModal">Add Doctor Type</button>
...
<tbody>
    <?php
        $sql = "SELECT * FROM doctor_types";
        $query = mysqli_query($db, $sql);

        if (!mysqli_num_rows($query) > 0) {
            echo '<td colspan="10"><center>No Doctor Types Data Found </center></td>';
        } else {
            while ($rows = mysqli_fetch_array($query)) {
                ...
                echo '<tr>
                    ...
                    <td>
                    <button class="btn btn-danger deleteBtn" data-toggle="modal" data-target="#deleteModal" data-id="' . $rows['dt_id'] . '">Delete</button>
                </td>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" id="closeDeleteModal">Close</button>
...
<div class="modal fade" id="addDoctorTypeModal" tabindex="-1" role="dialog" aria-labelledby="addDoctorTypeModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <!-- Form for adding a new doctor type -->
                <form id="addDoctorTypeForm">
                    <div class="form-group">
                        <label for="doctorTypeName">Doctor Type Name</label>
                        <input type="text" class="form-control" id="doctorTypeName" name="doctorTypeName" required>
                    </div>
                    <button type="button" class="btn btn-secondary" id="closeAddModal">Close</button>
                    <button type="submit" class="btn btn-primary">Add Doctor Type</button>
                </form>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function () {
        ...
        }, 100); // Change 5000 to the duration in milliseconds
    }

    // Function to show success message using showAlert function
    function showSuccessMessage(message) {
        showAlert(message, 'success');
    }

    // Function to show error message using showAlert function
    function showErrorMessage(message) {
        showAlert(message, 'error');
    }
        // Submit form to add a new doctor type
        $('#addDoctorTypeForm').on('submit', function (event) {
            event.preventDefault();
            var doctorTypeName = $('#doctorTypeName').val();

            $.ajax({
                type: 'POST',
                url: 'add_doctor_type.php',
                data: {
                    doctorTypeName: doctorTypeName
                },
                success: function (response) {
                    showSuccessMessage('Doctor type added successfully.');
                    setTimeout(function () {
                        location.reload();
                    }, 100);
                },
                error: function (error) {
                    showErrorMessage('Failed to add doctor type.');
                }
            });
        });
        ...
        // Close Add Doctor Type Modal and reload the page
        $('#closeAddModal').on('click', function () {
            $('#addDoctorTypeModal').modal('hide');
            location.reload();
        });

        // Close Delete Modal and reload the page
